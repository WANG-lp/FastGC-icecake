...
class FakeInputPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop,
                 shard_id, num_shards, dali_cpu=False):
        super(FakeInputPipe, self).__init__(batch_size,
                                            num_threads,
                                            device_id,
                                            seed=12 + device_id)
        self.external_data = ExternalFakeInputIter(
        ...
        print('FAKE INPUT!!!!!! DALI "{0}" variant'.format(dali_device))
...
def main():
    ...
    train_loader = DALIClassificationIterator(
        pipe, size=int(pipe.epoch_size("Reader") / args.world_size), last_batch_padded=True, fill_last_batch=False)
